...
function InterviewRunner() {
  ...
  if (!activeSession) return <div className="min-h-screen bg-black text-center py-20 text-slate-400">Loading...</div>;
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 pb-32">
      <div className="flex justify-between items-center bg-white/[0.04] p-6 rounded-2xl border border-white/10 mb-6">
        <div>
          ...
          <div className="flex gap-2 mt-2">
            {activeSession?.questions?.map((q, i) => (
              <div
                ...
                className={`w-3 h-3 rounded-full cursor-pointer transition-all ${i === currentQuestionIndex ? 'bg-teal-400 scale-125 ring-2 ring-teal-400/30' :
                  q.isEvaluated ? 'bg-emerald-400' :
                    (q.isSubmitted || submittedLocal[i]) ? 'bg-amber-400 animate-pulse' : 'bg-white/10'
                  }`}
              />
            ))}
          </div>
      ...
      <div className="bg-white/[0.04] border border-white/10 text-white p-8 rounded-3xl mb-6">
        <span className="text-teal-400 text-xs font-bold uppercase tracking-widest">Question {currentQuestionIndex + 1}</span>
        <h2 className="text-2xl mt-2 font-medium leading-relaxed">{currentQuestion?.questionText}</h2>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white/[0.04] p-6 rounded-3xl border border-white/10 flex flex-col items-center justify-center min-h-[300px]">
          <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6">Verbal Answer</h3>

          {!isRecording && !currentDraft.audioBlob ? (
            <button
              onClick={startRecording}
              disabled={isQuestionLocked}
              className="w-20 h-20 bg-teal-400 rounded-full flex items-center justify-center text-black shadow-xl hover:scale-105 transition-all disabled:opacity-50 disabled:bg-white/10 disabled:cursor-not-allowed"
            >
              🎤
            </button>
          ) : isRecording ? (
            <div className="text-center">
              ...
              <p className="mt-4 font-mono text-rose-400 font-bold">{recordingTime}s</p>
            </div>
          ) : (
            <div className="text-center">
              <div className="text-emerald-400 font-bold text-lg mb-2">Audio Captured ✅</div>
              {!isQuestionLocked && (
                <button onClick={() => setDrafts(prev => ({ ...prev, [currentQuestionIndex]: { ...prev[currentQuestionIndex], audioBlob: null } }))} className="text-xs text-slate-500 underline hover:text-rose-400">
                  Delete & Re-record
                </button>
              )}
            </div>
        ...
        <div className="bg-white/[0.04] p-2 rounded-3xl border border-white/10 overflow-hidden h-[400px]">
          <div className="flex justify-between px-4 py-2 bg-black/30 border-b border-white/10">
            <span className="text-xs font-bold text-slate-400 uppercase py-2">Code Editor</span>
            <select
              value={selectedLanguage}
              onChange={(e) => setSelectedLanguage(e.target.value)}
              disabled={isQuestionLocked}
              className="text-xs bg-black border border-white/10 text-white rounded-lg px-2 disabled:bg-white/5 disabled:text-slate-600"
            >
      ...
      {currentQuestion?.isEvaluated && (
        <div className="mt-6 bg-emerald-400/10 border border-emerald-400/20 p-6 rounded-2xl animate-in fade-in slide-in-from-bottom-4">
          <h3 className="text-emerald-300 font-bold mb-2">💡 AI Feedback</h3>
          <p className="text-emerald-200/80 text-sm leading-relaxed">{currentQuestion.aiFeedback}</p>
          <div className="mt-4 flex gap-4">
            <span className="bg-black/30 px-3 py-1 rounded-lg text-xs font-bold text-emerald-300">Score: {currentQuestion.technicalScore}/100</span>
          </div>
        </div>
      )}

      <div className="fixed bottom-0 left-0 right-0 bg-black/95 backdrop-blur-md border-t border-white/10 p-4 px-6 md:px-12 flex justify-between items-center z-50">
        <button
        ...
        <div className="flex flex-col items-center">
          ...
          {isProcessing && message && (
            <div className="mb-2 text-xs font-mono text-teal-300 bg-teal-400/10 px-3 py-1 rounded-full animate-pulse border border-teal-400/20">
              🤖 {message}...
            </div>
          )}
          ...
          <button
            ...
            disabled={isQuestionLocked}
            className={`px-8 py-3 rounded-xl font-bold shadow-lg transition-all ${isProcessing ? 'bg-white/10 text-slate-500 cursor-wait' :
              currentQuestion?.isEvaluated ? 'bg-emerald-500 text-white' :
                isQuestionLocked ? 'bg-white/10 text-slate-500' :
